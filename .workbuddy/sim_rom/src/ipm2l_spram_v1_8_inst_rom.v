...
//       end
    ...
    assign wr_byte_en_bus = (c_WR_BYTE_EN == 0) ? 8'hFF : 8'h00;
...
//                                                                                                    qb_data_bus[gen_i*Q_DATA_WIDTH+gen_j*Q_CAS_DATA_WIDTH +:Q_DATA_WIDTH_H] ,qa_data_bus[gen_i*Q_DATA_WIDTH+gen_j*Q_CAS_DATA_WIDTH +:Q_DATA_WIDTH_H]};
    ...
    //==== [sim patch] iverilog 不执行无敏感信号的 always@(*),这里显式给出 Modelsim 下的等效初值 ====
    initial begin
        cs_bit0      = 1'b0;
        cs_bit1_bus  = {ADDR_LOOP_NUM{1'b0}};
        cs_bit2_bus  = {ADDR_LOOP_NUM{1'b0}};
        csb_bit0     = 1'b0;
        csb_bit1_bus = {ADDR_LOOP_NUM{1'b0}};
        csb_bit2_bus = {ADDR_LOOP_NUM{1'b0}};
    end
